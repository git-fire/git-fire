...
func TestLoad_MigratesLegacyRegistry(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("USERPROFILE", tmpHome)

	newPath := filepath.Join(tmpHome, ".config", "git-fire", "repos.toml")
	legacyDir := filepath.Join(tmpHome, ".git-fire")
	if err := os.MkdirAll(legacyDir, 0o700); err != nil {
		t.Fatal(err)
	}
	legacyPath := filepath.Join(legacyDir, "repos.toml")
	content := `[[repos]]
...
`
	...
	reg, err := Load(newPath)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if len(reg.Repos) != 1 || reg.Repos[0].Path != "/tmp/legacy-repo" {
		t.Fatalf("unexpected registry after migrate: %+v", reg.Repos)
	}
	if _, err := os.Stat(newPath); err != nil {
		t.Fatalf("new registry file missing: %v", err)
	}
	if _, err := os.Stat(legacyPath); !os.IsNotExist(err) {
		t.Fatal("legacy repos.toml should be renamed away")
	}
	if _, err := os.Stat(legacyPath + ".migrated"); err != nil {
		t.Fatalf("expected legacy renamed to .migrated: %v", err)
	}
}
